...
const ADetailsDoctor = ({ mostrarEspecialidades, history, userLogout, specialities, mostrarDoctor, doctor_details, loading, editarDoctor, match }) => {
    ...
    const onSubmit = e => {
        ...
        if (!validator.isEmpty(`${formData.tuition}`)
            && validator.isLength(`${formData.tuition}`, { max: 9 })) {

            if (selectDefaultValue().length > 0) {
                ...
                if (selectedSpecialities.length > 0) {
                    ...
                    formData.specialties = selectDefaultValue().map(s => {
                        return { id: s.value }
                    });
                }
            ...
            setFormError("Ha habido un error al guardar los datos");
        }
    }
    ...
    const selectDefaultValue = e => {
        ...
        if (doctor) {
            doctor.specialties.map(s => {
                array.push({ label: s.name, value: s.id });
            });
        }
        return array;
    }
    ...
    const settearDataForm = () => {
        ...
        if (doctor) {
            ...
            setChecked(doctor.doctor.outstanding);
        }
    }
                                            ...
                                            <form onSubmit={onSubmit}>
                                                <div class="form-group">
                                                    <div class="custom-control custom-checkbox">
                                                        <input class="custom-control-input" id="outstanding" type="checkbox" name="outstanding" onChange={() => setChecked(!formData.outstanding)} defaultChecked={formData.outstanding} />
                                                    ...
                                                    <label for="facebook">Facebook</label><input class="form-control" id="facebook" type="text" placeholder="Facebook del medico" onChange={onChange} name="facebook" defaultValue={formData.facebook} />
                                                    <label for="linkedin">Linkedin</label><input class="form-control" id="linkedin" type="text" placeholder="Linkedin del medico" onChange={onChange} name="linkedin" defaultValue={formData.linkedin} />
                                                    ...
                                                    <label for="tuition">Matrícula</label><input class="form-control" id="tuition" type="number" placeholder="Matrícula del medico" onChange={onChange} name="tuition" defaultValue={formData.tuition} required maxLength="9" minLength="2" />
                                                    <label for="specialities">Especialidad</label>
                                                    <Select options={selectSpecialities()} defaultValue={selectDefaultValue()} id="specialities" placeholder="Especialidades" isMulti name="specialities" onChange={handleSpecialities} />
