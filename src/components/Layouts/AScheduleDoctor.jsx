...
const AScheduleDoctor = ({ history, mostrarTurnos, loading, error, turns, agregarTurno, turn_added, turn_deleted, eliminarTurno, turn_details, mostrarTurno, editarTurno, agregarDoctorTurn, mostrarDoctSelect, doctors, doctor_turns_added }) => {
     ...
     * Calendar DateRange
     */
    const [doctorTurnDate, setDoctorTurnDate] = useState([]);
    ...
    const handleDeleteTurn = id => {
        let answer = window.confirm("¿Eliminar turno?");
        if (answer) {
            eliminarTurno(id);
        }
    }
    ...
    const selectDoctors = () => {
        const list = []
        doctors.forEach(d => {
            list.push({ label: d.doctor.name, value: d.doctor.id });
        });
        return list;
    }
    ...
    const selectTurns = () => {
        const list = []
        turns.forEach(t => {
            list.push({ label: `${t.start_time} - ${t.final_hour}`, value: t.id });
        });
        return list;
    }

    const handleDoctors = args => {
        console.log(args);
        if (args) {
            setDoctorSelected({ ...doctorSelected, ['id']: args.value });
        }
    }

    const handleTurns = args => {
        console.log(args);
        if (args) {
            setTurnSelected({ ...turnSelected, ['id']: args.value });
        }
    }
                        ...
                        <div id="layoutSidenav_content">
                            ...
                            <main>
                                ...
                                <div class="container mt-n10">
                                    <div class="card mb-4">
                                        <div class="card-header"> {editing ? <>Editar turnos</> : <>Agregar turnos</>} </div>
                                        <div class="card-body">
                                            <div className="row">
...
export default connect(mapStateToProps, mapDispatchToProps)(AScheduleDoctor);